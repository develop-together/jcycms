...
use yii;
use common\components\BaseConfig as Constants;
    ...
    public function init()
    {
        ...
        if( !$this->content && $this->content !== false ) {
            $this->content = function ($model, $key, $index, $gridView) {
                ...
                if (!is_string($text)) throw new yii\base\InvalidArgumentException("No status valued {$model[$field]}");
                if ($this->url === false) {
                    $url = '';
                } else {
                    if ($this->url == '') {
                        $url = Url::to(['update', 'id' => $model['id'], 'type' => 'single']);
                    } else {
