        ...
        <div class="index_about">
            <?php if ($this->beginCache('cache_div_' . $model->id, ['duration' => 3600 * 24, 'dependency' => $dependency, 'enabled' => true])): ?>
                <div id="cache_div_<?= $model->id ?>">
                    ...
                    <p class="box_c">
                        <span class="d_time" id="d_time"><?= Yii::t('frontend', 'Created At') ?>
                            ：<?php //echo $model->created_at ?></span>
                        <span><?= Yii::t('frontend', 'Author') ?>：<?= $model->user->penname ?></span>
                        <span><?= Yii::t('frontend', 'View count') ?>：<b
                                    id="scan_count"><?php //echo $model->getScan_count() ?></b></span>
                    </p>
                    <div class="infos">
                        <?= $model->content ?>
                    </div>
                        ...
                        <p><span><?= Yii::t('frontend', 'Keywords') ?></span>：
                            <?php if ($model->tagFormat): ?>
                                <?php foreach ($model->tagFormat as $tag): ?>
                                    <a href="<?= Url::to(['article/tags', 'tag' => $tag]) ?>"
                                       target="_self"><?= $tag ?></a>
                                <?php endforeach ?>
                            <?php endif ?>
                        </p>
                    ...
                    <div class="nextinfo">
                        <?php if ($prevModel): ?>
                            <p><?= Yii::t('frontend', 'previous') ?>
                                ：<?= Html::a($prevModel->title, Url::to(['article/view', 'id' => $prevModel->id]), ['title' => $prevModel->sub_title, 'target' => '_self']) ?></p>
                        <?php endif ?>
                        <?php if ($nextModel): ?>
                            <p><?= Yii::t('frontend', 'next') ?>
                                ：<?= Html::a($nextModel->title, Url::to(['article/view', 'id' => $nextModel->id]), ['title' => $nextModel->sub_title, 'target' => '_self']) ?></p>
                        <?php endif ?>
                    </div>
                    <div class="otherlink">
                    ...
                    <div class="comt-title" style="display: block;">
                        <div class="comt-author left">
                            <?php if (Yii::$app->user->isGuest): ?>
                                <a class="switch-author" href="javascript:void(0)"
                                   style="font-size:12px;"><?= Yii::t('frontend', 'Get me logged in') ?></a>
                            <?php else: ?>
                                <a class="author" style="font-size:12px;"
                                   href="<?= $uCenterUrl ?>">Hi, <?= Yii::$app->getUser()->identity->username ?></a>
                            <?php endif ?>
                        </div>
                        <a id="cancel-comment-reply-link" class="right" href="javascript:;"
                           style="display: none;"><?= Yii::t('frontend', 'Cancel comment') ?></a>
                        <div class="clearfix"></div>
                    </div>
                    <!--评论区域 begin-->
                    <div class="reviewArea">
                        <div class="comment">
                            ...
                            <div class="comment_content">
                                <div class="cont-box">
                                    <textarea class="comment-input text" id="comment-textarea"
                                              placeholder="<?= $i18n5 ?>"></textarea>
                                </div>
                                <div class="tools-box">
                                    ...
                                    <li>
                                        ...
                                        <div class="reply-cont">
                                            ...
                                            <div style="float:left;padding-right: 5px;cursor: pointer;"
                                                 class="comment-hf hf-con-block" data-aid="<?= $comment->article_id ?>"
                                                 data-id="<?= $comment->id ?>" data-addcommented="0"><?= $i18n6 ?></div>
                                            　
                                            <div style="float:left;padding-right: 5px;" class="date-dz-z"
                                                 data-submited="0" data-id="<?= $comment->id ?>"><i
                                                        class="date-dz-z-click-red"></i><?= $i18n7 ?> (<i
                                                        class="z-num"><?= $comment->like_count ?></i>)
                                            </div>
                                                    ...
                                                    <?php foreach ($comment->childrens as $value): ?>
                                                        <li style="border:none">
                                                            ...
                                                            <div class="reply-cont">
                                                                ...
                                                                <div style="float:left;padding-right: 5px;cursor: pointer;"
                                                                     class="comment-hf hf-con-block"
                                                                     data-aid="<?= $value->article_id ?>"
                                                                     data-id="<?= $value->id ?>"
                                                                     data-addcommented="0"><?= $i18n6 ?></div>
                                                                　
                                                                <div style="float:left;padding-right: 5px;"
                                                                     class="date-dz-z" data-id="<?= $value->id ?>"
                                                                     data-submited="0"><i
                                                                            class="date-dz-z-click-red"></i><?= $i18n7 ?>
                                                                    (<i class="z-num"><?= $value->like_count ?></i>)
                                                                </div>
